...
export const QuizView: React.FC<QuizViewProps> = ({ progress, onUpdateProgress }) => {
  ...
  // Generate 8-10 randomized quiz questions from VOCAB_DATABASE
  const generateQuestions = (mode: QuizMode) => {
    ...
    const questions: QuizQuestion[] = selectedVocab.map((targetWord) => {
  ...
  }, [currentQuestionIndex, quizMode, currentQ, hasAnswered, quizFinished]);
  ...
  const handleNextQuestion = () => {
    if (currentQuestionIndex + 1 < quizQuestions.length) {
      ...
      const finalScore = score + (selectedOptionIndex !== null && currentQ.options[selectedOptionIndex].isCorrect ? 1 : 0);
    ...
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      <div className="bg-white rounded-2xl p-4 border border-stone-200 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-bold text-stone-900 text-base">認字與台語中文諧音推敲測驗</h2>
        </div>
      ...
      {!quizFinished ? (
          ...
          <div className="bg-white rounded-3xl p-6 sm:p-8 border border-stone-200/90 shadow-md space-y-6">
            <div className="flex items-center justify-between border-b border-stone-100 pb-4">
              <div className="flex items-center gap-2">
                ...
                {categoryMeta && (
                  <span className={`text-xs px-2.5 py-0.5 rounded-full font-medium border ${categoryMeta.badgeColor}`}>
                    {categoryMeta.label}
                  </span>
                )}
              </div>
            ...
            <div className="text-center py-4 space-y-4">
              {quizMode === 'listening' ? (
                <div className="space-y-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 px-6 py-3.5 rounded-2xl bg-linear-to-r from-rose-500 to-indigo-600 hover:from-rose-600 hover:to-indigo-700 text-white font-bold text-base shadow-lg shadow-rose-500/25 active:scale-95 transition-all"
                  >
              ...
              {!hasAnswered && (
                <div>
                  {showHint ? (
                    <div className="inline-flex items-center gap-1.5 p-2.5 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-900">
              ...
              {currentQ.options.map((opt, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-2xl border text-left transition-all duration-200 flex items-start justify-between gap-3 ${btnStyle} cursor-pointer`}
                  >
            ...
            {hasAnswered && (
              <div className="rounded-2xl p-4 sm:p-5 bg-stone-900 text-white space-y-4 animate-fadeIn">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {currentQ.word.syllablesBreakdown.map((s, sIdx) => (
                      <div
                        key={sIdx}
                        className="bg-stone-800 px-3 py-2 rounded-xl border border-stone-700 flex items-center gap-2 text-xs"
                      >
                ...
                <div className="pt-2 flex justify-end">
                  ...
                  >
                    <span>{currentQuestionIndex + 1 < quizQuestions.length ? '下一題' : '查看測驗成績'}</span>
        ...
        <div className="bg-white rounded-3xl p-6 sm:p-10 border border-stone-200 shadow-xl text-center space-y-6">
          ...
          <div className="space-y-1">
            <h3 className="text-2xl font-black text-stone-900">恭喜完成認字測驗！</h3>
            <p className="text-sm text-stone-600">
              你利用台語與中文的發音線索，成功挑戰了 {quizQuestions.length} 個生活基礎韓文單字
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-stone-900 hover:bg-stone-800 text-white font-bold text-sm shadow-md transition-all active:scale-95 cursor-pointer"
            >
              <RotateCcw className="w-4 h-4" />
              <span>再來一回隨機測驗 (10 題)</span>
            </button>
          </div>
        </div>
      )}
